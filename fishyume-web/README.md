...
# 重启目标 profile 后，侧栏 footer 出现 Fishyume 入口，可打开原生工作区面板
```
...
- **client 面**（`src/client/plugin.tsx`）：向 `sidebar.footer.action` 和
  `shell.overlay` 注册原生 React 入口与工作区，不创建 iframe 或独立 SPA 外壳。
...
- 待验证：在真实 web profile 中刷新页面确认 sidebar footer 与原生工作区渲染（需浏览器）。
