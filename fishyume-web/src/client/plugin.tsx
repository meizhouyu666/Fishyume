...
type Team = { teamId: string; project?: string; topic?: string; status?: string; state?: string; mode?: string; stateVersion?: number; costGrant?: number; costUsed?: number; participants?: number | TeamParticipant[]; createdAt?: string | number; updatedAt?: string }
...
.dsh-fishyume-team-subnav-divider{width:1px;height:20px;margin:0 6px;background:var(--fy-border)}.dsh-fishyume-team-subnav-tab{height:30px;padding:0 10px;border:0;border-radius:6px;color:var(--fy-text-secondary);background:transparent;font:600 13px/18px inherit;cursor:pointer}.dsh-fishyume-team-subnav-tab:hover{color:var(--fy-text-primary);background:var(--fy-bg-hover)}.dsh-fishyume-team-subnav-tab[aria-current="page"]{color:var(--fy-text-primary);background:var(--fy-bg-elevated)}
...
async function loadRouting(): Promise<void> {
  ...
  try {
    ...
    updatePanel({ drivers: drivers.drivers ?? [], routes: routes.routes ?? [], loading: false })
  } catch (error) { updatePanel({ loading: false, error: error instanceof Error ? error.message : String(error) }) }
}
...
function LocalizedWorkspace({ state }: { state: PanelState }): JSX.Element {
  ...
  return <div className="dsh-fishyume-workspace">
    ...
    {!state.loading && !state.error && state.view === 'routing' ? <div className="dsh-fishyume-columns"><div className="dsh-fishyume-list" aria-label="执行器列表">{state.drivers.length === 0 ? <div className="dsh-fishyume-state">暂无执行器。</div> : state.drivers.map((driver) => <div key={driver.driver} className="dsh-fishyume-driver"><strong>{driver.driver}</strong><small>{driver.available ? '可用' : driver.diagnostic || '不可用'}</small></div>)}</div><section className="dsh-fishyume-detail" aria-label="生效路由"><span className="dsh-fishyume-eyebrow">路由</span><h3>生效路由</h3>{state.routes.length === 0 ? <p>暂无团队路由配置。</p> : state.routes.map((route) => <p key={route.routeId}><strong>{route.routeId}</strong><br /><small>{route.driver || '未知'} / {route.provider || '默认'} / {route.model || '默认'} · {route.enabled ? '已启用' : '已停用'}</small></p>)}</section></div> : null}
  </div>
}
...
function TeamSubnav({ state }: { state: PanelState }): JSX.Element {
  ...
  return <nav className="dsh-fishyume-team-subnav" aria-label="团队子导航">
    ...
    <span className="dsh-fishyume-team-subnav-divider" aria-hidden="true" />
    <button type="button" className="dsh-fishyume-team-subnav-tab" aria-current={state.teamSection === 'tasks' ? 'page' : undefined} onClick={() => choose('tasks')}>团队任务</button>
    <button type="button" className="dsh-fishyume-team-subnav-tab" aria-current={state.teamSection === 'templates' ? 'page' : undefined} onClick={() => choose('templates')}>团队模板</button>
  </nav>
}
...
function TeamTemplateEditor({ template, capabilities }: { template?: TeamTemplate; capabilities?: TeamCapabilities }): JSX.Element {
  ...
  const copyLaunch = (): void => { void navigator.clipboard?.writeText(`启动 fishyume 的${draft.name || '团队'}`) }
