...
当前 `0.2.1-alpha.1` 已完成 M4.0 + M4.1 + M4.2：TypeScript CLI/TUI 会自动发现、按需启动并连接用户级 Go Control Plane。Windows 使用 Named Pipe，Linux/macOS 使用 Unix Domain Socket；直接启动 `wf-engine` 时仍保留 stdio JSON-RPC。Node Attempt 使用 Agent Driver、Headless Protocol v1 与 Context Compiler v1；CC-Panes 已退出新 Run，仅保留历史 snapshot 读取和不可恢复诊断。Agent-native MCP/Application API（M4.3）尚未实现。
...
当前仍不支持正式 MCP/Application API 产品面、通用 Shell/HTTP/容器节点、模型回退或动态节点。M4.2 不包含 Web/Desktop、Memory、模型路由、Prompt Library、Native Harness、Claude Driver 或第三方 Driver SDK；动态发现与运行时热加载后置到核心控制面稳定之后。
