...
async function main() {
  ...
  try {
    ...
    const started = await bridge.call('run.start', {project, workflow, clientRequestId: `codex-live-${Date.now()}`});
    const deadline = Date.now() + 180000;
    let result;
    while (Date.now() < deadline) {
      try {result = await bridge.call('run.result', {runId: started.runId}); break}
      catch (error) {
        if (error?.data?.code !== 'not_ready') throw error;
      }
      await new Promise(resolveWait => setTimeout(resolveWait, 1000));
    }
    if (!result) throw new Error(`Codex live Run ${started.runId} timed out`);
    const node = result.results.find(item => item.nodeId === 'agent');
    if (result.conclusion !== 'succeeded' || node?.result?.summary !== 'codex-engine-live-smoke') {
