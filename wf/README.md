...
M4.4 now includes the product/migration release surface, deterministic and real Codex MCP Host flows, local real-Codex single-node and parallel Driver smokes, a rendered Host/PTY handoff with stale-action conflict, a two-client MCP Host/TUI-controller acceptance, and Provider-independent Application crash/restart acceptance. The technical gates are complete and final independent review remains; a real Provider crash record is optional supplemental evidence. Public CI never requires Provider credentials.
