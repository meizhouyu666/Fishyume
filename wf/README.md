...
M4.4 now includes the product/migration release surface, a deterministic MCP Host flow, a local real-Codex single-node smoke, a two-client MCP Host/TUI-controller concurrency acceptance test, and a Provider-independent Application crash/restart acceptance. A real Host model plus rendered PTY session, a parallel real-Driver Workflow, a real Provider/live crash record, and final independent review remain pending; public CI never requires Provider credentials.
