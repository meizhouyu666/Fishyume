...
On an interactive terminal, `fishyume run` uses the responsive Ink Run Console with explicit non-color status markers, parallel Attempt and Approval panels, diagnostics, progress, and keyboard actions. Reattach with `fishyume status <run-id> --watch`; use `j`/`k` or arrows to select, `a` to approve, `r` to reject with a reason, `R` to confirm retry, `c` to confirm cancellation, `?` for help, and `Esc` to leave an input or confirmation mode. Indeterminate retries require an explicit duplicate-risk acknowledgement. `d`, `q`, and `Ctrl+C` detach a started run or stop watch-only observation without implicitly cancelling it.

The console targets 80/120/160 columns and degrades from TrueColor to ANSI or monochrome. Non-TTY/CI output remains the stable line-oriented reporter. Non-interactive `--watch` is rejected with an actionable diagnostic, `--watch --json` is invalid, and plain `fishyume status --json` remains a single machine-readable object.
