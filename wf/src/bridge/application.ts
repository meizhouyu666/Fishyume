...
export interface WorkflowValidateResponse {apiVersion: typeof applicationApiVersion; workflowSchemaVersion: string; valid: boolean; issues: ValidationIssue[]; capabilityGaps: ValidationIssue[]; warnings: string[]; routingRequirements: RoutingRequirementView[]}
export interface WorkflowExplainResponse {apiVersion: typeof applicationApiVersion; workflowSchemaVersion: string; name: string; topologicalOrder: string[]; parallelLayers: string[][]; nodes: Array<{id: string; type: string; dependsOn: string[]; parallelLayer: number; approvalPrompt?: string; condition?: Record<string, unknown>; contextSources: string[]; projectInstructions?: string[]; memoryBindings?: MemoryBinding[]; contextPolicyVersion: string; routing?: RoutingRequirement; agent?: {driver: string; target: string}}>; capabilityGaps: ValidationIssue[]; warnings: string[]}
