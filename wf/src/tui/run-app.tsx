...
import {actionableNodes, approveAction, basicConsoleKeyEvent, boundActionableNode, consolePanelLines, initialConsoleInteractionState, resumeActionForMode, selectedActionableNode, transitionConsoleState} from './interaction.js';
...
export function RunApp({view, startedAt, width: fixedWidth, now: fixedNow, colorMode: fixedColorMode, onResume, onCancel, onExit}: RunAppProps) {
  ...
  const targets = actionableNodes(view); const target = selectedActionableNode(interaction, targets); const boundTarget = boundActionableNode(interaction, targets); const interactive = Boolean(onResume && onCancel && onExit);
  const targetSignature = targets.map(item => `${item.nodeId}:${item.kind}:${item.duplicateRisk}`).join('|');
  useEffect(() => {dispatch({type: 'reconcile', targets})}, [targetSignature]);
  ...
  const currentAction = (): ResumeAction | undefined => {
    const action = resumeActionForMode(interaction, targets);
    if (!action) {setMessage('target is no longer actionable'); dispatch({type: 'reconcile', targets})}
    return action;
  };
  useInput((input, key) => {
    if (!run || !interactive || pending) return;
    const basicEvent = basicConsoleKeyEvent(input, key, targets);
    if (basicEvent?.type === 'escape') {dispatch(basicEvent); return}
    if (interaction.mode === 'reject') {
      if (key.return) {const action = currentAction(); if (action) void submitResume(action); return}
    ...
    if (interaction.mode === 'retry-confirm' || interaction.mode === 'retry-risk-confirm') {
      if (key.return || input.toLowerCase() === 'y') {const action = currentAction(); if (action) void submitResume(action)}
      return;
    }
  ...
  const panelTarget = interaction.mode === 'reject' || interaction.mode === 'retry-confirm' || interaction.mode === 'retry-risk-confirm' ? boundTarget : target;
  ...
  return <Box flexDirection="column" width={width}>
    ...
    <Section title="WORKFLOW" width={width} colorMode={colorMode}>{nodes.map(node => {const actionable = targets.some(item => item.nodeId === node.id); const marker = interactive ? target?.nodeId === node.id ? '> ' : actionable ? '· ' : '  ' : ''; return <NodeRow key={node.id} node={node} width={contentWidth} colorMode={colorMode} marker={marker}/>})}</Section>
    ...
    {interactive ? <Section title="RUN CONSOLE" width={width} colorMode={colorMode}>{consolePanelLines(interaction, panelTarget, contentWidth, pending, message).map((line, index) => <Text key={index}>{line}</Text>)}</Section> : <HelpFooter view={view} width={width} colorMode={colorMode}/>}
  </Box>;
}
