...
const tools: Array<{name: FishyumeMethod; description: string; inputSchema: Record<string, z.ZodTypeAny>}> = [
  ...
  {name: 'run.start', description: 'Create or replay one durable Workflow Run. In an interactive Host, immediately follow with web.open targeting the returned Run and let the Web client observe execution. Do not poll run.events in an LLM loop unless the user explicitly requests live progress or a pending action requires Host input; after handoff, return the runId/attach information and yield control. Reuse the same clientRequestId only with the same request payload. For fishyume/v2, provide explicit contextBindings for host-selected Memory IDs.', inputSchema: {project: z.string(), workflow, inputs: z.record(z.string(), scalar).optional(), driver: z.string().optional(), target: z.string().optional(), clientRequestId: z.string(), contextBindings: contextBindings.optional()}},
  ...
  {name: 'run.events', description: 'Read durable events after a sequence using bounded pagination and an optional bounded wait. This is an on-demand control-plane read, not a background watcher: use it once when the user asks for progress or when an action/terminal transition is expected, then stop rather than repeatedly polling from the Host Agent.', inputSchema: {runId: z.string(), afterSequence: z.number().int().nonnegative().optional(), limit: z.number().int().optional(), waitMs: z.number().int().nonnegative().optional()}},
  ...
  {name: 'run.result', description: 'Retrieve the bounded structured terminal result after a terminal event or an explicit user request. A non-terminal Run returns not_ready; yield control instead of retrying in a Host Agent polling loop.', inputSchema: {runId: z.string()}},
  ...
  {name: 'team.capabilities', description: 'Discover the independent fishyume.team/v1 Panel, deterministic model templates, limits, catalog hash, and Driver capability gates.', inputSchema: {schemaVersion: teamVersion}},
  {name: 'team.start', description: 'Create or replay one durable read-only multi-model Panel and dispatch its initial independent contributions. In an interactive Host, follow with web.open targeting the returned Team so the optional Web client follows the same task. Reuse clientRequestId only with the same exact request.', inputSchema: {schemaVersion: teamVersion, clientRequestId: z.string().min(1).max(128), project: z.string().min(1).max(4096), topic: z.string().min(1).max(16 * 1024), instructions: z.string().max(16 * 1024).optional(), participants: z.array(participant).min(2).max(4).optional(), costGrant: z.number().int().min(1).max(6400).optional()}},
  ...
  {name: 'team.action', description: 'Submit an idempotent whole-Team cancellation with the latest stateVersion.', inputSchema: {...teamIdentity, actionId: z.string().min(1).max(128), expectedStateVersion: z.number().int().positive(), type: z.enum(['cancel'])}},
  ...
  {name: 'driver.inventory', description: 'Probe installed Codex, Claude Code, and OpenCode CLIs for installed, version, authenticated, and exposed-model facts. Surface-neutral; does not call a model.', inputSchema: {schemaVersion: z.literal(configApiVersion)}},
