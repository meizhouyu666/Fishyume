...
const tools: Array<{name: ApplicationMethod; description: string; inputSchema: Record<string, z.ZodTypeAny>}> = [
  ...
  {name: 'workflow.validate', description: 'Validate the exact fishyume/v2 start intent without running it. Pass the same workflow, inputs, driver, target, and explicit contextBindings to explain and start; static issues, effective Agent routing requirements, and Driver capability gaps are separate.', inputSchema: workflowRequest},
  {name: 'workflow.explain', description: 'Preflight the exact start intent without mutation: DAG layers, explicit Context Policy dependencies/instructions/Memory bindings, effective Agent routing requirements, resolved Driver/target, gaps, and warnings. Routing requirements are declarative in M6.2; no model is selected. Pass this same payload to run.start plus clientRequestId.', inputSchema: workflowRequest},
