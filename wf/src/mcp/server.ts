...
const tools: Array<{name: ApplicationMethod; description: string; inputSchema: Record<string, z.ZodTypeAny>}> = [
  {name: 'system.capabilities', description: 'First Host call: discover the bounded versioned authoringGuide, fishyume/v2 Workflow schema, limits, actions, and current Driver/target readiness.', inputSchema: {project: z.string().optional()}},
  ...
  {name: 'run.get', description: 'Read authoritative durable Run, Node, Attempt, bounded Context metadata, result, and action preconditions. Use its latest stateVersion and attempt for run.action.', inputSchema: {runId: z.string()}},
