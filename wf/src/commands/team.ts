...
export class TeamStartCommand extends Command {
  ...
  static usage = Command.Usage({description: 'Run a durable read-only multi-model Panel.'});
  ...
  async execute(): Promise<number> {
    try {
      ...
      return startTeam(new EngineBridge(), {schemaVersion: teamApiVersion, clientRequestId: `team-start-${randomUUID()}`, project: this.project ?? process.cwd(), topic: this.topic, instructions: this.instructions, participants, costGrant}, {detach: this.detach, json: this.json}, this.context.stdout);
