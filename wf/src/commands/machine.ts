...
import {callRouting, RoutingCallError, routingMethods, type RoutingMethod} from '../bridge/routing.js';

const methods = new Set<string>([...applicationMethods, ...teamMethods, ...routingMethods]);
...
export async function runMachine(client: EngineClient, method: string, paramsText: string | undefined, output: {write(text: string): unknown}): Promise<number> {
  try {
    ...
    const response = routingMethods.includes(method as RoutingMethod) ? await callRouting(client, method as RoutingMethod, params) : method.startsWith('team.') ? await callTeam(client, method as TeamMethod, params) : await callApplication(client, method as ApplicationMethod, params);
    output.write(`${JSON.stringify(response)}\n`);
    return 0;
  } catch (error) {
    const applicationError = error instanceof ApplicationCallError ? error.applicationError : error instanceof TeamCallError ? error.teamError : error instanceof RoutingCallError ? error.routingError : {code: 'invalid_argument', message: error instanceof Error ? error.message : String(error)};
...
export class MachineCommand extends Command {
  ...
  static usage = Command.Usage({description: 'Call one Agent-facing Application, Team, or Routing API method and print one JSON response.'});
