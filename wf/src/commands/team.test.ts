...
import {parseParticipant, renderTeam, startTeam} from './team.js';
...
function team(state: 'running' | 'closed') {return {schemaVersion: 'fishyume.team/v1', teamId: 'team-1', clientRequestId: 'request-1', requestHash: 'c'.repeat(64), project: 'C:/project', topic: 'Compare designs', catalogHash: 'a'.repeat(64), participants: [{participantId: 'participant-1', label: 'architect', role: 'propose', modelId: 'codex/local/gpt-5.6', driver: 'codex', target: 'local', state: 'responded'}], state, stateVersion: state === 'closed' ? 4 : 2, costGrant: 1000, costUsed: 100, ...(state === 'closed' ? {closeReason: 'panel_settled'} : {}), createdAt: now, updatedAt: now}}
...
test('team start waits for the Panel and renders independently identified contributions', async () => {
  ...
  const code = await startTeam(client, {schemaVersion: 'fishyume.team/v1', clientRequestId: 'request-1', project: 'C:/project', topic: 'Compare designs'}, {pollMs: 1}, {write(text) {output += text}});
...
test('team start detach returns the durable identity without observing', async () => {
  const client = new TeamClient(); let output = '';
  assert.equal(await startTeam(client, {schemaVersion: 'fishyume.team/v1', clientRequestId: 'request-1', project: 'C:/project', topic: 'Compare'}, {detach: true, json: true}, {write(text) {output += text}}), 0);
