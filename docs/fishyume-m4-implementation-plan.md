...
- [x] 用真实 Codex Host Agent 通过 MCP 创建和运行 Workflow（`smoke:codex-host-mcp`，无人工 allow，read-only sandbox）。
- [x] 用真实 Codex Driver 执行并行 Agent → Approval → Agent Workflow（通过 `smoke:codex-live:parallel`，真实 Provider gate 不进入公共 CI）。
- [x] 用真实 Host Agent 与 rendered PTY TUI 重复并发操作（`smoke:codex-host-pty`；TUI 动作成功，Host 陈旧动作 `conflict`，双方收敛终态）。
- [x] Provider-independent Control Plane crash/restart acceptance（正式 Application API、action receipt replay、active Attempt 不重复）；真实 Provider/live crash 仅作可选补充证据。
